...
def _cdist_forward_kernel(
    ...
    # Use the backend's round-to-nearest sqrt so p=2 distances agree with the
    # native FP32 implementation instead of the generic fast approximation.
    result = libdevice.sqrt_rn(tl.sum(acc, axis=0))
...
def _cdist_backward_kernel(
    ...
    for n2_start in range(0, n2, BLOCK_N2):
        ...
        # Match the native p=2 derivative: zero at coincident points and a
        # round-to-nearest divide elsewhere.  Adding an epsilon changes the
        # derivative for every nonzero distance and accumulates across n2.
        contrib = tl.div_rn(grad[:, None] * diff, cdist[:, None])
        contrib = tl.where(cdist[:, None] == 0.0, 0.0, contrib)
...
def _cdist_backward(grad, x1, x2, p, cdist):
    ...
    # Keep the fixed 64-wide reduction used by the tuned PPU path.  The
    # reduction tree is part of the measured numerical contract; changing the
    # width for short n2 values increases the H100 difference on the Protenix
    # shape rather than reducing it.
    BLOCK_N2 = 64
    # Keep the coordinate-sized tile so each point uses one compact program;
    # this avoids the vector-width reassociation of the old 64-lane tile.
    BLOCK_DIM = min(triton.next_power_of_2(dim), 4)
