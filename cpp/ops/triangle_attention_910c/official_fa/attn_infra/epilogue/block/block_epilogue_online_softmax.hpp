...
public:
    ...
    BlockEpilogue(Arch::Resource<ArchTag> &resource, float scaleValue_)
    {
        ...
        constexpr uint32_t PSE_UB_TENSOR_OFFSET = 6 * UB_UINT8_BLOCK_SIZE;
        constexpr uint32_t BIAS2_UB_TENSOR_OFFSET = 8 * UB_UINT8_BLOCK_SIZE;
        ...
        pseUbTensor = resource.ubBuf.template GetBufferByByte<float>(PSE_UB_TENSOR_OFFSET);
        bias2UbTensor = resource.ubBuf.template GetBufferByByte<float>(BIAS2_UB_TENSOR_OFFSET);
    }
    ...
    __aicore__ inline
    void RowsumSPECTILE384(const AscendC::LocalTensor<float> &srcUb, const AscendC::LocalTensor<float> &rowsumUb,
        const AscendC::LocalTensor<float> &tvUbTensor, uint32_t numRowsRound, uint32_t numElems,
        uint32_t numElemsAligned)
    {
        AscendC::BlockReduceSum<float, false>(
    ...
    __aicore__ inline
    void RowmaxSPECTILE384(const AscendC::LocalTensor<float> &srcUb, const AscendC::LocalTensor<float> &rowmaxUb,
        const AscendC::LocalTensor<float> &tvUbTensor, uint32_t numRowsRound, uint32_t numElems,
        uint32_t numElemsAligned)
    {
        AscendC::BlockReduceMax<float, false>(
    ...
    __aicore__ inline
    void CopyPseGmToUb(AscendC::LocalTensor<float> dst, AscendC::GlobalTensor<float> src,
        uint32_t dstOffset, uint32_t rowNum, uint32_t columnNum, uint32_t pseStride)
    {
        uint32_t columnNumRound = NpuArch::Detail::Alignment::RoundUp(columnNum, BLOCK_SIZE);
        constexpr uint32_t BLOCK_BYTES = BLOCK_SIZE_IN_BYTE;
        constexpr uint32_t ELEMENT_BYTES = sizeof(float);
        const uint32_t blockLen = (columnNum * ELEMENT_BYTES + BLOCK_BYTES - 1) / BLOCK_BYTES;

        AscendC::Duplicate<float>(dst[dstOffset], 0.0f,
            static_cast<uint64_t>(rowNum) * columnNumRound);
        if ((pseStride * ELEMENT_BYTES) % BLOCK_BYTES == 0) {
            AscendC::DataCopyParams copyParams;
            copyParams.blockCount = rowNum;
            copyParams.blockLen = blockLen;
            copyParams.srcStride = (pseStride * ELEMENT_BYTES - blockLen * BLOCK_BYTES) / BLOCK_BYTES;
            copyParams.dstStride = columnNumRound * ELEMENT_BYTES / BLOCK_BYTES - blockLen;
            AscendC::DataCopy(dst[dstOffset], src, copyParams);
        } else {
            AscendC::DataCopyPad(
                dst[dstOffset], src,
                AscendC::DataCopyExtParams(
                    rowNum, columnNum * ELEMENT_BYTES,
                    (pseStride - columnNum) * ELEMENT_BYTES,
                    (columnNumRound - columnNum) * ELEMENT_BYTES / BLOCK_BYTES, 0),
                AscendC::DataCopyPadExtParams<float>(false, 0, 0, 0));
        }
    }

    __aicore__ inline
    void CopyBias1GmToUb(AscendC::LocalTensor<float> dst, AscendC::GlobalTensor<float> src,
        uint32_t dstOffset, uint32_t rowNum, uint32_t columnNum, uint32_t biasStride)
    {
        uint32_t columnNumRound = NpuArch::Detail::Alignment::RoundUp(columnNum, BLOCK_SIZE);
        CopyPseGmToUb(dst, src, dstOffset, 1, columnNum, biasStride);
        AscendC::PipeBarrier<PIPE_ALL>();
        for (uint32_t rowIdx = 1; rowIdx < rowNum; ++rowIdx) {
            AscendC::DataCopy<float>(
                dst[dstOffset + rowIdx * columnNumRound], dst[dstOffset], columnNumRound);
        }
        AscendC::PipeBarrier<PIPE_ALL>();
    }

    __aicore__ inline
    void AddBiasUbToUb(AscendC::LocalTensor<float> biasUbTensor,
        uint32_t sUbOffset, uint32_t rowNumCurLoop, uint32_t columnNumRound, uint32_t biasUbOffset)
    {
        AscendC::Add<float, false>(
            lsUbTensor[sUbOffset],
            lsUbTensor[sUbOffset],
            biasUbTensor[biasUbOffset],
            (uint64_t)0,
    ...
    __aicore__ inline
    void AddPseToUb(uint32_t sUbOffset, uint32_t rowNumCurLoop, uint32_t columnNumRound,
        uint32_t pseUbOffset)
    {
        AddBiasUbToUb(pseUbTensor, sUbOffset, rowNumCurLoop, columnNumRound, pseUbOffset);
    }
    ...
    {
        if (columnNumPad >= columnNumRound) {
            AscendC::DataCopy(
                gOutput,
                lpUbTensor[sUbOffset],
                AscendC::DataCopyParams(
                    rowNumCurLoop, columnNumRound / BLOCK_SIZE, 0,
            ...
            AscendC::DataCopyPad(
                ...
                lpUbTensor[sUbOffset],
                AscendC::DataCopyExtParams(
                    rowNumCurLoop, columnNumPad * sizeof(ElementOutput), 0, 0, 0));
        }
    }
    ...
    void operator()(AscendC::GlobalTensor<ElementOutput> gOutput, AscendC::GlobalTensor<ElementInput> gInput,
        ...
        uint32_t isFirstStackTile, uint32_t isLastNoMaskStackTile,
        uint32_t qSBlockSize, uint32_t qNBlockSize, uint32_t curStackTileMod,
        AscendC::GlobalTensor<float> gPse, uint64_t pseOffset, uint32_t pseStride)
    {
        uint32_t rowNum = actualBlockShape.m();
        uint32_t columnNum = actualBlockShape.n();
        uint32_t columnNumRound = NpuArch::Detail::Alignment::RoundUp(columnNum, BLOCK_SIZE);
        ...
        // PSE uses the independent LO buffer. EVENT_ID6 is also the official
        // rescale-O reuse fence for this buffer.
        ...
        for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoopNum; rowLoopIdx++) {
            ...
            int64_t offsetInput = layoutInput.GetOffset(MatrixCoord(rowOffsetIoGm, 0));
            auto gInputCurLoop = gInput[offsetInput];
            auto gPseCurLoop = gPse[pseOffset + static_cast<uint64_t>(rowOffsetIoGm) * pseStride];

            AscendC::WaitFlag<AscendC::HardEvent::V_MTE2>(pingpongFlag);
            CopySGmToUb(
                gInputCurLoop, (pingpongFlag * MAX_UB_S_ELEM_NUM), rowNumCurLoop, columnNumRound, columnNumPad);
            CopyPseGmToUb(
                pseUbTensor, gPseCurLoop, (pingpongFlag * MAX_UB_S_ELEM_NUM),
                rowNumCurLoop, columnNum, pseStride);
            AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(pingpongFlag);
            AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(pingpongFlag);

            uint32_t offsetOutput = layoutOutput.GetOffset(MatrixCoord(rowOffsetIoGm, 0));
            auto gOutputCurLoop = gOutput[offsetOutput];
            auto layoutOutputCurLoop = layoutOutput.GetTileLayout(MatrixCoord(rowNumCurLoop, columnNum));
            AddPseToUb(
                (pingpongFlag * MAX_UB_S_ELEM_NUM), rowNumCurLoop, columnNumRound,
    ...
    void operator()(AscendC::GlobalTensor<ElementOutput> gOutput, AscendC::GlobalTensor<ElementInput> gInput,
        ...
        uint32_t isFirstStackTile, uint32_t isLastNoMaskStackTile,
        uint32_t qSBlockSize, uint32_t qNBlockSize, uint32_t curStackTileMod,
        AscendC::GlobalTensor<float> gBias1, uint64_t bias1Offset, uint32_t bias1Stride, uint32_t bias1Rows,
        AscendC::GlobalTensor<float> gBias2, uint64_t bias2Offset, uint32_t bias2Stride)
    {
        uint32_t rowNum = actualBlockShape.m();
        uint32_t columnNum = actualBlockShape.n();
        uint32_t columnNumRound = NpuArch::Detail::Alignment::RoundUp(columnNum, BLOCK_SIZE);
        ...
        // Bias1 and Bias2 share the independent LO buffer, so keep its rescale-O fence at tile scope.
        AscendC::WaitFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID6);
        for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoopNum; rowLoopIdx++) {
            uint32_t pingpongFlag = 0;
            uint32_t rowOffsetCurLoop = rowLoopIdx * rowNumTile;
            ...
            int64_t offsetInput = layoutInput.GetOffset(MatrixCoord(rowOffsetIoGm, 0));
            auto gInputCurLoop = gInput[offsetInput];
            uint64_t bias1RowOffset = (bias1Rows == 1) ? 0 :
                static_cast<uint64_t>(rowOffsetIoGm) * bias1Stride;
            auto gBias1CurLoop = gBias1[bias1Offset + bias1RowOffset];
            auto gBias2CurLoop = gBias2[bias2Offset +
                static_cast<uint64_t>(rowOffsetIoGm) * bias2Stride];
            ...
            if (bias1Rows == 1) {
                CopyBias1GmToUb(
                    pseUbTensor, gBias1CurLoop, sUbOffset,
                    rowNumCurLoop, columnNum, bias1Stride);
            } else {
                CopyPseGmToUb(
                    pseUbTensor, gBias1CurLoop, sUbOffset,
                    rowNumCurLoop, columnNum, bias1Stride);
            ...
            AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(pingpongFlag);
            AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(pingpongFlag);
            AddPseToUb(sUbOffset, rowNumCurLoop, columnNumRound, sUbOffset);

            AscendC::PipeBarrier<PIPE_ALL>();
            CopyPseGmToUb(
                bias2UbTensor, gBias2CurLoop, sUbOffset,
                rowNumCurLoop, columnNum, bias2Stride);
            AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(pingpongFlag);
            AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(pingpongFlag);
            AddBiasUbToUb(bias2UbTensor, sUbOffset, rowNumCurLoop, columnNumRound, sUbOffset);
...
private:
    ...
    AscendC::LocalTensor<float> pseUbTensor;
    AscendC::LocalTensor<float> bias2UbTensor;
};
}
